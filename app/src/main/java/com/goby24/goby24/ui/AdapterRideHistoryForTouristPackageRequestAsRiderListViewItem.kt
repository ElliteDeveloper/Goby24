...
class AdapterRideHistoryForTouristPackageRequestAsRiderListViewItem(context:Context):BaseAdapter() {
    private val mInflater: LayoutInflater = context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater
    private var mContext:Context = context
    private var mArrData:ArrayList<RideDataForTouristPackageRequestAsRiderModel> = ArrayList()

    fun setData(context: Context, arrData:ArrayList<RideDataForTouristPackageRequestAsRiderModel>) {
        mContext = context
        mArrData = ArrayList()
        if (arrData.size > 0){
    ...
    override fun getView(position: Int, convertView: View?, p2: ViewGroup?): View {
        ...
        val item: RideDataForTouristPackageRequestAsRiderModel = mArrData[position]
