...
class AdapterRideHistoryForBasicAsRiderListViewItem(context:Context):BaseAdapter() {
    private val mInflater: LayoutInflater = context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater
    private var mContext:Context = context
    private var mArrData:ArrayList<RideDataForBasicRideAsRiderModel> = ArrayList()
    private var mStrForAction = mContext.getString(R.string.ride_completed)
    private var mStrAction = mContext.getString(R.string.review)

    fun setData(context: Context, arrData:ArrayList<RideDataForBasicRideAsRiderModel>) {
        mContext = context
        mArrData = ArrayList()
        if (arrData.size > 0){
    ...
    override fun getView(position: Int, convertView: View?, p2: ViewGroup?): View {
        ...
        val item: RideDataForBasicRideAsRiderModel = mArrData[position]
