...
class AdapterTouristPackageRequestFromTouristLvItem(listdata: ArrayList<TouristPackageRequestFromTouristModel>, ctx: Context, lv: ListView):
    BaseAdapter() {
    ...
    private var mLvData:ListView = lv
...
//    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun getView(position: Int, p1: View?, p2: ViewGroup?): View {
        var itemView = p1
        if (itemView == null){
            itemView = mInflater.inflate(R.layout.rvitem_tourist_package_all_request, null)
            var vh = ViewHolder(itemView)
            itemView.tag = vh
        }
